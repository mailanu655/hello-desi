...
from app.services.session_store import message_seen
...
async def handle_message(
    ...
    Pipeline order (intentional):
      ...
      4. Non-text guard (images/audio/etc. → graceful response)
      5. Rate limiting
      6. First-time user onboarding
      7. Active session check (BEFORE greeting — prevents mid-flow interruption)
      8. Returning user greeting
      9. Intent routing (registration → deals → monetization → digest → report)
    ...
    # ── 5. Rate limiting (Supabase-backed) ──────────────────────
    ...
    # ── 6. First-time user onboarding (Supabase-backed) ─────────
    ...
    # ── 7. Active session check (BEFORE greeting) ───────────────
    # This prevents "hi" from breaking a user's mid-registration/deal/upgrade flow
    ...
    # ── 8. Returning user greeting (AFTER session check) ────────
    msg_lower = message_body.lower().strip()
    greetings = {"hi", "hello", "hey", "hola", "namaste", "start", "help"}
